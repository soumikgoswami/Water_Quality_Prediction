...
# Function to load data and preprocess (as in the notebook)
@st.cache_data
def load_and_preprocess_data(file_path):
    ...
    # Sort by 'id' and 'date' for correct lag calculation
    df = df.sort_values(by=['id', 'date']).copy()

    # Impute missing values with the mean (as per your notebook's last step for imputation)
    ...
    # Drop the original date column and other columns not used for training
    df = df.drop(['date', 'NH4', 'BSK5', 'Suspended'], axis=1) # Drop original date and other non-target pollutants

    return df

# Function to train and save the model (only run if model files don't exist)
@st.cache_resource
def train_and_save_model(df):
    ...
    # Ensure all relevant columns are used as features, including new lagged and seasonal features
    # Exclude the target pollutants themselves from X, but include their lagged versions
...
def main():
    ...
    # Get unique station IDs from the dataframe for the dropdown
    ...
    if st.sidebar.button("Predict Water Quality"):
        ...
        # To get lagged features for the prediction, we need the last known values for that station.
        # This is a simplification; in a real-world scenario, you might need a more robust way
        # to get the actual previous measurements for the selected station.
        # For this example, we'll use the last available values from the training data for the selected station.
        ...
        # One-hot encode 'id' for the input data, aligning with trained columns
        ...
        for col in model_columns:
            if col not in input_encoded.columns:
                input_encoded[col] = 0
        input_encoded = input_encoded[model_columns] # Ensure column order is the same
